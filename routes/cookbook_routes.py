    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            if "ollama" in req.cmd:
                ...
                ps_lines.append('# Auto-install llama.cpp — prefer a GPU-accelerated prebuilt binary for AMD,')
                ps_lines.append('# fall back to the CPU pip wheel if no AMD GPU is present or download fails.')
                ...
                ps_lines.append('  } else {')
                ps_lines.append('    # Detect AMD GPU; sort by VRAM descending so dGPU wins over iGPU')
                ps_lines.append('    $amdGpu = Get-CimInstance Win32_VideoController | Where-Object { $_.Name -match "AMD|Radeon|Ryzen AI" } | Sort-Object AdapterRAM -Descending | Select-Object -First 1')
                ps_lines.append('    if ($amdGpu) {')
                ps_lines.append('      $isRdna3Plus = $amdGpu.Name -match "RX [79]\\d{3}|Ryzen AI Max"')
                ps_lines.append('      $asset = if ($isRdna3Plus) { "win-hip-radeon-x64" } else { "win-vulkan-x64" }')
                ps_lines.append('      Write-Host "AMD GPU detected ($($amdGpu.Name)) — downloading llama.cpp $asset binary..."')
                ps_lines.append('      try {')
                ps_lines.append('        $rel = Invoke-RestMethod "https://api.github.com/repos/ggml-org/llama.cpp/releases/latest" -UseBasicParsing')
                ps_lines.append('        $tag = $rel.tag_name')
                ps_lines.append('        $zip = "$env:TEMP\\llama-$asset.zip"')
                ps_lines.append('        $url = "https://github.com/ggml-org/llama.cpp/releases/download/$tag/llama-$tag-bin-$asset.zip"')
                ps_lines.append('        Invoke-WebRequest $url -OutFile $zip -UseBasicParsing')
                ps_lines.append('        Expand-Archive $zip "$env:LOCALAPPDATA\\llama-cpp\\" -Force')
                ps_lines.append('        $env:PATH = "$env:LOCALAPPDATA\\llama-cpp\\bin;$env:PATH"')
                ps_lines.append('        Write-Host "llama-server installed ($tag, $asset)."')
                ps_lines.append('      } catch {')
                ps_lines.append('        Write-Host "Binary download failed — falling back to pip install..."')
                ...
                ps_lines.append('      }')
                ps_lines.append('    } else {')
                ps_lines.append('      Write-Host "Installing llama-cpp-python (CPU)..."')
                ps_lines.append('      python -m pip install llama-cpp-python[server] --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cpu')
                ps_lines.append('    }')
                ps_lines.append('  }')
                ps_lines.append('}')
